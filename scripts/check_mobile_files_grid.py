...
async def run():
    ...
    old=os.getcwd();os.chdir(ROOT);srv=http.server.ThreadingHTTPServer(('127.0.0.1',PORT),H);threading.Thread(target=srv.serve_forever,daemon=True).start()
    ...
    try:
        ...
        for _ in range(40):
            try:
                tabs=json.load(urllib.request.urlopen(f'http://127.0.0.1:{PORT+1}/json/list'));tab=next(t for t in tabs if t.get('type')=='page');break
