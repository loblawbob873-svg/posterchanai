...
const rows=[]; for(let i=0;i<24;i++){const sha=String(i).padStart(64,'0'),pdf=i===0,name=pdf?'Annual-report.pdf':`IMG_${String(i).padStart(4,'0')}_summer-holiday-photo.jpg`,mime=pdf?'application/pdf':'image/jpeg'; rows.push({sha256:sha,url:'https://media.example/'+sha,type:mime,size:102400+i}); FilesIdx.rows[sha]={name,folder:FilesIdx.folders()[i%6],mime};}
