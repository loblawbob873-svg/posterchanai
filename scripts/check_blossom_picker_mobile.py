...
const ME={pubkey:'phone-user'}, _MIME_EXT={'image/jpeg':'jpg'}, FilesIdx={_lastIndexSha:'index',loadLocal(){},async ensure(){},meta(s){return this.rows[s]||{}},folderOf(s){return this.meta(s).folder||''},folders(){return ['Camera','Screenshots from an exceptionally long journey','Receipts','Family','Wallpapers','Downloads']},isEncFolder(){return false},rows:{}};
...
AUDIT = r'''(()=>{const q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)],r=e=>e.getBoundingClientRect(),vis=e=>e&&r(e).width>0&&r(e).height>0, folders=qa('.folder-chip').filter(vis),cards=qa('.bp-pick-card').filter(vis),grid=q('#bp-grid'),side=q('#bp-folders');const out={errors:window.__errors||[],overflow:document.documentElement.scrollWidth>innerWidth+1,folders:folders.length,cards:cards.length,vertical:folders.length>1&&Math.abs(r(folders[0]).x-r(folders[1]).x)<2&&r(folders[1]).y>r(folders[0]).y,folderDims:folders.map(x=>[Math.round(r(x).width),Math.round(r(x).height),x.clientWidth,x.scrollWidth]),folderReadable:folders.every(x=>r(x).width>=200&&r(x).height>=36&&x.scrollWidth<=x.clientWidth+2),sideScroll:side.scrollHeight>side.clientHeight,gridScroll:grid.scrollHeight>grid.clientHeight,fileReadable:cards.every(x=>r(x).height>=154&&r(x).width>=120&&x.querySelector('.fname')&&x.querySelector('small')&&x.querySelector('img')),compressed:cards.some(x=>r(x).height<150)};if(cards[0])cards[0].click();out.picked=!!window.__picked;return out;})()'''
...
async def run():
    ...
    try:
        ...
        async with websockets.connect(tab['webSocketDebuggerUrl']) as ws:
            ...
            required={'errors':[], 'overflow':False,'folders':7,'cards':24,'vertical':True,'folderReadable':True,'sideScroll':True,'gridScroll':True,'fileReadable':True,'compressed':False,'picked':True}
