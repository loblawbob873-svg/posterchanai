...
class WayfireWM{
  ...
  async snap(id,zone){const row=(await this.windows()).find(x=>x.id===Number(id));if(!row)return false;const outs=await this.outputs(),cx=row.rect.x+row.rect.width/2,cy=row.rect.y+row.rect.height/2,o=outs.find(o=>cx>=o.rect.x&&cx<o.rect.x+o.rect.width&&cy>=o.rect.y&&cy<o.rect.y+o.rect.height)||outs[0];if(!o)return false;
    const wa=o.work,b=wa?{x:wa.x,y:wa.y,width:wa.w,height:wa.h}:o.rect,h=wa?b.height:Math.max(1,b.height-72),half=Math.floor(b.width/2);let x=b.x,y=b.y,w=b.width,hh=h;if(zone==='left')w=half;else if(zone==='right'){x+=half;w=b.width-half;}else if(/^(top|bottom)-(left|right)$/.test(zone)){const p=zone.split('-'),halfH=Math.floor(h/2);w=p[1]==='left'?half:b.width-half;if(p[1]==='right')x+=half;hh=p[0]==='top'?halfH:h-halfH;if(p[0]==='bottom')y+=halfH;}else if(zone!=='max')return false;// configure-view sizes the content, while the taskbar must stay below the decorated frame.
