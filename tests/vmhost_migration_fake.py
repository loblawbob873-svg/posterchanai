...
`httpx.ASGITransport`. Only three things are fixtures, each the far end of a real protocol:
  ...
  * `MigratingFakeBackend` — FakeBackend plus the migration primitives (inactive XML, snapshots,
    undefine-for-migration). `define` PARSES the XML it is given, so the path rewriting is observable;
...
class MigratingFakeBackend(FakeBackend):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.snapshots: dict = {}        # uuid -> [{"name","xml","current"}]
    ...
    async def dumpxml_inactive(self, vm_uuid):
        ...
        # What libvirt reports carries the CURRENT metadata, not the XML's original element.
        ...
        if d["meta_xml"]:
            ...
            i = out.find("</uuid>") + len("</uuid>")
            out = out[:i] + f"<metadata>{meta.to_xml(prefixed=True)}</metadata>" + out[i:]
        return out

    async def snapshot_names(self, vm_uuid):
        await self._enter("snapshot_names", vm_uuid)
        snaps = self.snapshots.get(vm_uuid, [])
        return [s["name"] for s in snaps], next((s["name"] for s in snaps if s["current"]), "")
    ...
    async def snapshot_redefine(self, vm_uuid, xml, workdir, current=False):
        ...
        if current:
            for s in snaps:
                s["current"] = False
        snaps.append({"name": name, "xml": xml, "current": bool(current)})
