...
const p={
  relayQuery:async()=>[membership],relayQueryFrom:async(relays,filters)=>{listedQueries.push({relays,filters});if(filters[0].kinds[0]===10009)return [membership];return [];},
  verifyRelayEvents:async events=>{verifyCalls++;return events;},nip04dec:async()=>{nip04Calls++;throw new Error('not nip04');},
...
p.relayQueryFrom=async(relays,filters)=>{listedQueries.push({relays,filters});return [metadataEvent,{...metadataEvent,id:'forged-extra',tags:[['d','other']]}];};
...
p.relayQueryFrom=async(relays,filters)=>{listedQueries.push({relays,filters});return [chat,{...chat,id:'wrong',tags:[['h','other']]}];};p.profOf=()=>({name:'Alice'});
...
if(!listedQueries.slice(-2).every(q=>q.relays.length===1&&q.relays[0]==='wss://groups.example')||verifyCalls<3)throw new Error(`listed-relay reads were not signature verified (${verifyCalls})`);
