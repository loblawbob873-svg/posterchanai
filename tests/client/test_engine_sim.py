"""Puts engine_sim.js — the reconciler's own suite — INTO the suite.
...
none. pytest collects .py files, so `node engine_sim.js` passing on a laptop meant nothing about any
...
SIM = os.path.join(ROOT, "tests", "client", "engine_sim.js")
...
class EngineSimTests(unittest.TestCase):
    ...
    def test_no_case_has_gone_missing(self):
        ...
        with open(SIM, encoding="utf-8") as fh:
            n = len(re.findall(r"^T\('", fh.read(), re.M))
        self.assertGreaterEqual(n, 21, "engine_sim.js lost test cases (%d left)" % n)
    ...
    def test_the_harness_can_actually_fail(self):
        ...
        with open(SIM, encoding="utf-8") as fh:
            src = fh.read()
        at = src.index("if(fail.length)")
        broken = src[:at] + "T('sentinel', (ck) => ck(false, 'forced'));\n" + src[at:]
