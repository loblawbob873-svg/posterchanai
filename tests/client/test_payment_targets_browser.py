...
def page():
    ...
    functions='\n'.join(function(n) for n in ('isXmrAddr','_tipMethodSheet','doTip','doZap','doXmrTip'))
    script=r'''
...
const sign=async(kind,content,tags,created_at)=>{signCalls++;if(signMode==='reject')throw new Error('Signer refused');const event=NostrTools.finalizeEvent({kind,content,tags:signMode==='change'?[['payto','lightning','wrong@test.test']]:tags,created_at},key);if(holdSign)await new Promise(r=>releaseSign=r);return event;};
let profile={name:'Alice'};const profOf=()=>profile,xmrOf=p=>p.monero_address||'',bchOf=()=>'',isBchAddr=()=>false,xmrForNote=()=>'';
...
const _lightningAmountSheet=(_p,go)=>go(21),_runZap=(_id,pk,amount,address)=>{lastTip={pk,amount,address};};
const doBchTip=(_pk,address)=>{lastTip={address};};
...
@pytest.mark.parametrize('case',['hydrate','save','clear','publish_failure','offline','account_switch','conflict','chooser','monero_tip','lightning_choice','invalid_monero','signer_refusal','signer_changed_targets','chooser_account_switch','cold_outbox','lazy_module','legacy_card','pinned_card','stored_note'])
