...
const source=clientSourceAt(process.argv[2]),start=source.indexOf('  let _userSettingsRender='),end=source.indexOf('    _usMail =',start);
(async()=>{for(const activeSigner of [true,false]){
 ...
 for(const phase of ['fetch','backoff']){
  ...
  vm.createContext(installStateGlobals(ctx) && ctx);vm.runInContext(source.slice(start,end)+'return {loaded:s};}',ctx);
