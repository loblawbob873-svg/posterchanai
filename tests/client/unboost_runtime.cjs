...
function setup(saved){
 const storage=saved||new Map(),ctx={console,crypto:crypto.webcrypto,TextEncoder,TextDecoder,Uint8Array,ArrayBuffer,Buffer,setTimeout,clearTimeout,URL,Map,Set,Date};ctx.window=ctx;ctx.localStorage={getItem:k=>storage.get(k)||null,setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)};ctx.addEventListener=()=>{};ctx.document={querySelectorAll:()=>[]};vm.createContext(installStateGlobals(ctx) && ctx);vm.runInContext(fs.readFileSync(root+'/static/vendor/nostr/nostr.bundle.js','utf8'),ctx);vm.runInContext(fs.readFileSync(root+'/static/js/client/store.js','utf8'),ctx);
