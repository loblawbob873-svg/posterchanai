...
__PC.switchView('mail');''')
        ...
        # A hung page belongs to its old list, not the newly restored inbox.
        await browser.js("window.oldMailPage=pendingMailPage;var input=document.querySelector('#mail-search');input.value='';input.dispatchEvent(new Event('input',{bubbles:true}));")
        await browser.until("!!document.querySelector('#mail-more-btn')")
        await click(browser,'#mail-more-btn')
        await browser.until('pendingMailPage!==oldMailPage')
        await browser.js('oldMailPage()')
        await browser.js('new Promise(r=>setTimeout(r,30))')
        assert await browser.js("document.querySelector('#mail-more-btn').disabled")
        # Invoke the actual handler despite disabled UI to verify the request ownership guard.
        await browser.js("document.querySelector('#mail-more-btn').onclick()")
        assert await browser.js("mailRequests.filter(p=>p.includes('until=')).length===2")
        await browser.js('pendingMailPage()')
        await browser.until("document.querySelector('#mail-items').textContent.includes('stale page')")
        await browser.js("var input=document.querySelector('#mail-search');input.value='invoice';input.dispatchEvent(new Event('input',{bubbles:true}));")
        await browser.until("document.querySelectorAll('.mail-item').length===2&&!document.querySelector('#mail-items').textContent.includes('stale page')")
        await browser.js('new Promise(r=>setTimeout(r,30))')
