...
def test_zapstore_publisher_creates_clean_current_signer_listing():
    workflow = (ROOT / ".github/workflows/android.yml").read_text()
    patch = (ROOT / "scripts/zsp-signing-lineage.patch").read_text()

    assert "ZSP_ANDROID_PREVIOUS_CERTIFICATE_HASHES" not in workflow
    ...
    assert "Zapstore AppCatalog rejected the clean APK asset" in workflow
