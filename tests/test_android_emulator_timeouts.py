...
def test_lifecycle_and_instrumented_device_checks_have_independent_bounds():
    ...
    assert "timeout --foreground 15m bash scripts/android_device_checks.sh; a=$?" in WORKFLOW
    assert "timeout --foreground 25m bash scripts/android_instrumented.sh; b=$?" in WORKFLOW
    assert "device=$a instrumented=$b" in WORKFLOW


def test_device_diagnostics_still_upload_after_a_timeout():
    tail = WORKFLOW.split("timeout --foreground 15m", 1)[1]
...
def test_first_activity_launch_cannot_hang_the_entire_device_gate():
    ...
    assert "timeout --foreground 30s adb shell am start -W" in DEVICE
