...
(function(){
  ...
  function render(){
    ...
    const directMessages=p.$('#messages-direct');if(directMessages)directMessages.onclick=()=>p.switchView('messages');
    if(current){
  ...
  function bind(me){
    ...
    const banMember=async target=>{ const rooms=saved(),room=rooms[state.community],viewer=p.viewer?p.viewer():{},bundle=room&&room.cord&&room.cord.bundle,reader=window.PosterCordReader,loadKey=room&&(room.communityId||room.naddr),wraps=roomControls.get(loadKey); if(!bundle||!reader||!reader.createBanWrap||!wraps)return p.toast('community moderation is not ready'); if(typeof window.confirm==='function'&&!window.confirm('Ban this member from the community?'))return; try{ const made=await reader.createBanWrap(bundle,wraps,target,viewer.pubkey,p.signTemplate),relays=[...new Set([...(bundle.relays||[]),...CORD_RELAYS])].slice(0,8),accepted=await p.relayPublishTo(relays,made.wrap); if(!accepted)throw new Error('community relays rejected the ban'); room.banned=made.banned; rooms[state.community]=room; save(rooms); render(); p.toast('member banned'); }catch(e){p.toast('member was not banned: '+(e&&e.message||e));} };
    const closeMemberMenu=()=>{const old=document.querySelector('.cc-member-menu');if(old)old.remove();};
    const openMemberMenu=(event,target)=>{closeMemberMenu();const canBan=isOwner&&target!==viewer.pubkey,menu=document.createElement('div');menu.className='cc-member-menu';menu.setAttribute('role','menu');menu.innerHTML=`<button data-cc-member-profile="${p.enc(target)}" role="menuitem">View profile</button>${canBan?`<button class="danger" data-cc-member-ban="${p.enc(target)}" role="menuitem">Ban from community</button>`:''}`;document.body.appendChild(menu);const x=Math.min(event.clientX||12,window.innerWidth-190),y=Math.min(event.clientY||12,window.innerHeight-(canBan?92:50));menu.style.left=Math.max(8,x)+'px';menu.style.top=Math.max(8,y)+'px';menu.querySelector('[data-cc-member-profile]').onclick=()=>{closeMemberMenu();if(p.openProfile)p.openProfile(target);};const ban=menu.querySelector('[data-cc-member-ban]');if(ban)ban.onclick=()=>{closeMemberMenu();void banMember(target);};setTimeout(()=>document.addEventListener('pointerdown',closeMemberMenu,{once:true}),0);};
    $$('[data-cc-member]').forEach(row=>{const target=row.dataset.ccMember;row.onclick=e=>{e.preventDefault();openMemberMenu(e,target);};row.oncontextmenu=e=>{e.preventDefault();openMemberMenu(e,target);};let held=null;row.onpointerdown=e=>{if(e.pointerType==='mouse')return;held=setTimeout(()=>{held=null;openMemberMenu(e,target);},550);};row.onpointerup=row.onpointercancel=row.onpointermove=()=>{if(held){clearTimeout(held);held=null;}};});
    ...
    $$('[data-cc-channel]').forEach(b=>b.onclick=async()=>{ const community=state.community,channel=b.dataset.ccChannel; state.channel=channel; mobileChatOpen=true; mobileDrawerOpen=false; render(); scrollChatBottom(); const rooms=saved(),room=rooms[community]; if(room&&room.cord&&!room.cord.hydrated){ try{ await hydrateRoomStreams(p,community); }catch(e){ p.toast('could not load room history: '+(e&&e.message||e)); } } if(state.community===community&&state.channel===channel)scrollChatBottom(); });
