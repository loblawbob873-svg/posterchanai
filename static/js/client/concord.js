...
(function(){
  ...
  function webxdcOf(m,room,channelName){
    ...
    for(const t of (m&&m.tags||[]).slice(0,100)){ if(t[0]!=='imeta')continue; const f={}; for(let i=1;i<Math.min(t.length,30);i++){ const s=String(t[i]||'').slice(0,4096),j=s.indexOf(' '); if(j>0)f[s.slice(0,j)]=s.slice(j+1); } const mime=(f.m||'').toLowerCase();/* Armada publishes Webxdc archives with the registered vendor MIME while older PosterChan messages used application/x-webxdc. Rejecting Armada's MIME discarded its explicit `webxdc` topic and made the raw-URL fallback invent a different session UUID, so both clients opened the same archive in different ioquake rooms. */if((mime==='application/vnd.webxdc+zip'||mime==='application/x-webxdc')&&/^https?:\/\//i.test(f.url||''))return {url:f.url,sha:f.x||'',uuid:f.webxdc||(scope?scope+'|webxdc':messageId(m)),name:(f.summary||f.name||'Mini app').slice(0,80),transport}; }
