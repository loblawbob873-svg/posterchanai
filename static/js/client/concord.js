...
(function(){
  ...
  if(!document.querySelector('link[data-concord-css]')){
    ...
    l.href='/static/css/concord.css?v=7'; (document.head||document.documentElement).appendChild(l);
  }
  const PC=()=>window.__PC;
  const state={ community:null, channel:null };
  let discovered=[], discoveryStarted=false, discoveryLoaded=false;
  ...
  function startDiscovery(p){
    ...
    try{ p.relaySubscribe([{kinds:[1],search:'armada.buzz/invite',limit:100},{kinds:[1],search:'poster.place/invite',limit:100}],{onEvent,onEose,live:true}); }catch(_){ discoveryLoaded=true; }
  }
  function render(){
    ...
    feed.innerHTML=`<div class="cc-app">
      <aside class="cc-communities"><div class="cc-brand" title="Concord" aria-label="Concord"><span aria-hidden="true">🕊</span></div>${rooms.map((r,i)=>`<button class="cc-server${state.community===i?' active':''}" data-cc-server="${i}" title="${p.enc(roomName(r,i))}">${roomIcon(p,r,i)}</button>`).join('')}<button class="cc-server cc-add" id="cc-add" title="Join a community">+</button></aside>
      <aside class="cc-channels"><header><button class="cc-mobile-back" id="cc-back-communities" aria-label="Communities">‹</button><div><b>${state.community==null?'Concord':p.enc(roomName(current,state.community))}</b><small>${current&&current.local?'Local test community':'End-to-end encrypted'}</small></div>${current?'<button class="cc-head-btn" id="cc-edit-icon" title="Set community icon" aria-label="Set community icon"><svg class="ic"><use href="#i-image"></use></svg></button>':''}<button class="cc-head-btn" id="cc-invite" title="Join with invite">+</button></header>
        <div class="cc-channel-list">${state.community==null?'<div class="cc-empty-side">Choose or join a community</div>':`<div class="cc-section">TEXT CHANNELS</div><button class="cc-channel active" data-cc-channel="general"><span>#</span> general</button>`}</div>
      ...
      <main class="cc-conversation"><header><button class="cc-mobile-back" id="cc-back-channels" aria-label="Channels">‹</button><span class="cc-hash">#</span><b>${state.channel||'general'}</b><span class="cc-visibility ${channelPrivate?'private':'public'}">${channelPrivate?'Private':'Public'}</span><span class="cc-topic">${p.enc((current&&current.description)||(channelPrivate?'Invite-only channel':'Visible to all community members'))}</span><span class="cc-spacer"></span>${current?'<button class="cc-head-btn" id="cc-copy-link" title="Copy room invite link" aria-label="Copy room invite link"><svg class="ic"><use href="#i-link"></use></svg></button><button class="cc-head-btn" id="cc-call" title="Start voice call"><svg class="ic"><use href="#i-phone"></use></svg></button>':''}<button class="cc-head-btn" id="cc-members" title="Members"><svg class="ic"><use href="#i-users"></use></svg></button></header>
        <div class="cc-messages">${state.community==null?`<div class="cc-discover"><div class="concord-mark">C</div><h2>Find your community</h2><p>Join an Armada-compatible CORD-05 invite or create a public relay community.</p><div class="cc-primary-actions"><button class="btn btn-neon" id="cc-create">Create community</button><button class="btn btn-ghost" id="cc-welcome-join">Join with invite</button></div><section class="cc-public"><div><h3>Public communities</h3><small>Public CORD invites discovered on your Nostr relays</small></div>${discovered.length?discovered.map((r,i)=>`<button data-cc-discover="${i}" class="cc-public-room"><b>${p.enc(r.name)}</b><span>${p.enc(r.description||'Public Concord invite')}</span></button>`).join(''):(discoveryLoaded?'<div class="cc-public-empty"><b>No public communities found</b><span>Publish or paste a public Armada/CORD invite to list it.</span></div>':'<div class="cc-public-empty"><b>Searching relays…</b><span>Looking for public Armada/CORD invite notes.</span></div>')}</section></div>`:(messages.length?`<div class="cc-message-list">${messages.map(m=>{const mp=p.profOf?p.profOf(m.pubkey):{};return `<article class="cc-message"><img class="cc-message-avatar" src="${p.enc(mp.picture||p.LOGO||'')}" alt=""><div class="cc-message-body"><b>${p.enc(m.by)}</b><time>${new Date(m.at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</time><p>${p.linkify?p.linkify(m.text):p.enc(m.text)}</p>${p.linkCardHtml?p.linkCardHtml(m.text):''}</div></article>`;}).join('')}</div>`:`<div class="cc-welcome"><div class="cc-welcome-hash">#</div><h2>Welcome to #${state.channel||'general'}</h2><p>${current&&current.local?'This local test room lets you validate the chat UI before publishing or joining a relay community.':'This is the start of this encrypted channel.'}</p></div>`)}</div>
        <div class="cc-reply hidden" id="cc-reply"></div><div class="cc-compose"><button class="cc-compose-btn" id="cc-attach" title="Attach file"><svg class="ic"><use href="#i-paperclip"></use></svg></button><input type="file" id="cc-file" multiple hidden><textarea id="cc-input" rows="1" placeholder="Message #${state.channel||'general'}" ${state.community==null?'disabled':''}></textarea><button class="cc-compose-btn" id="cc-emoji" title="Emoji"><svg class="ic"><use href="#i-smile"></use></svg></button><button class="btn btn-neon" id="cc-send" ${state.community==null?'disabled':''}>Send</button></div>
  ...
  function bind(me){
    ...
    const createGo=$('#cc-create-go'); if(createGo)createGo.onclick=async()=>{ const name=String($('#cc-community-name').value||'').trim(); if(!name){ p.toast('name your community'); return; } const viewer=p.viewer?p.viewer():{}; if(!viewer.pubkey||!window.PosterCord){ p.toast('sign in before creating a relay community'); return; } createGo.disabled=true; try{ p.toast('creating encrypted community…'); const relays=(p.relayUrls?p.relayUrls():[]).slice(0,3); const made=await window.PosterCord.createCommunity({name,icon:normalizeIcon($('#cc-community-icon').value),owner:viewer.pubkey,relays,base:location.origin,signEvent:p.signTemplate}); for(const ev of made.events){ const result=await p.relayPublish(ev); if(result&&result.ok===false)throw new Error(result.msg||'relay rejected event'); } await p.publish(1,`${name}\n\n${made.url}`,[['t','concord'],['t','community']]); const a=saved(); a.push({name,icon:normalizeIcon($('#cc-community-icon').value),description:'',channels:[{name:'general',private:false,id:made.generalChannelId}],local:false,naddr:inviteParts(made.url).naddr,url:made.url,cord:made}); save(a); state.community=a.length-1; state.channel='general'; render(); p.copyValue(made.url); p.toast('public community created — invite link copied'); }catch(e){ createGo.disabled=false; p.toast('community creation failed: '+(e&&e.message||e)); } };
    ...
    const copyLink=$('#cc-copy-link'); if(copyLink)copyLink.onclick=()=>{ const room=saved()[state.community]; if(!room||!room.url){ p.toast('Local test rooms do not have a relay invite link yet'); return; } p.copyValue(room.url); };
