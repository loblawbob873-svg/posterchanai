...
(function(){
  ...
  async function deriveWebxdcUrlTopic(url,messageId){const lower=String(url||'').toLowerCase();let source=String(url||'');for(let at=lower.lastIndexOf('.xdc');at>0;at=lower.lastIndexOf('.xdc',at-1)){const next=source[at+4];if(next===undefined||next==='?'||next==='#'||/\s/.test(next)){source=source.slice(0,at+4);break;}}const bytes=new Uint8Array(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(`webxdc-url-realtime-v1:${source}:${messageId}`))),alphabet='ABCDEFGHIJKLMNOPQRSTUVWXYZ234567';let out='',buf=0,bits=0;for(const b of bytes){buf=(buf<<8)|b;bits+=8;while(bits>=5){bits-=5;out+=alphabet[(buf>>>bits)&31];}}if(bits)out+=alphabet[(buf<<(5-bits))&31];return out;}
  function mintWebxdcTopic(){const alphabet='ABCDEFGHIJKLMNOPQRSTUVWXYZ234567',bytes=crypto.getRandomValues(new Uint8Array(32));let out='',buf=0,bits=0;for(const b of bytes){buf=(buf<<8)|b;bits+=8;while(bits>=5){bits-=5;out+=alphabet[(buf>>>bits)&31];}}if(bits)out+=alphabet[(buf<<(5-bits))&31];return out;}
