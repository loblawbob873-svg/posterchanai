...
(function(){
  ...
  function webxdcOf(m,room,channelName){
    ...
    for(const t of (m&&m.tags||[])){ if(t[0]!=='imeta')continue; const f={}; for(let i=1;i<t.length;i++){ const s=String(t[i]||''),j=s.indexOf(' '); if(j>0)f[s.slice(0,j)]=s.slice(j+1); } if((f.m||'').toLowerCase()==='application/x-webxdc'&&/^https?:\/\//i.test(f.url||''))return {url:f.url,sha:f.x||'',uuid:f.webxdc||(scope?scope+'|webxdc':messageId(m)),name:(f.summary||f.name||'Mini app').slice(0,80),transport}; }
    /* Armada's missing-imeta-id fallback is the deterministic scope default, not the chat event id. */
    const url=(String(m&&m.text||'').match(/https?:\/\/[^\s<>]+\.xdc(?:\?[^\s<>]*)?/i)||[])[0]; return url?{url,sha:'',uuid:scope?scope+'|webxdc':messageId(m),name:'Mini app',transport}:null;
  }
  ...
  function hydrateWebxdcCards(room){ if(!window.PCWebxdc||!PCWebxdc.cardHtml||!document.querySelectorAll)return; const byId=new Map(activeMessages(room).map(m=>[messageId(m),m])); document.querySelectorAll('.cc-message[data-message-id]').forEach(el=>{ const m=byId.get(el.dataset.messageId),html=m&&webxdcHtml(m,room,state.channel),body=el.querySelector('.cc-message-body'); if(html&&body&&!body.querySelector('.xdc-card'))body.insertAdjacentHTML('beforeend',html); }); }
  ...
  function bind(me){
    ...
    const banMember=async target=>{ const initial=saved(),room=initial[state.community],roomId=roomIdentity(room),viewer=p.viewer?p.viewer():{},bundle=room&&room.cord&&room.cord.bundle,reader=window.PosterCordReader,loadKey=room&&(room.communityId||room.naddr),wraps=roomControls.get(loadKey); if(!bundle||!reader||!reader.createBanWrap||!wraps)return p.toast('community moderation is not ready'); if(typeof window.confirm==='function'&&!window.confirm('Ban this member from the community?'))return; try{ const made=await reader.createBanWrap(bundle,wraps,target,viewer.pubkey,p.signTemplate),relays=[...new Set([...(bundle.relays||[]),...CORD_RELAYS])].slice(0,8),accepted=await p.relayPublishTo(relays,made.wrap); if(!accepted)throw new Error('community relays rejected the ban');/* A signer may keep this promise open while the owner changes rooms. Update the moderated room by durable identity instead of overwriting the newly active numeric index. */const latest=saved(),roomIndex=latest.findIndex(item=>roomIdentity(item)===roomId);if(roomIndex<0)throw new Error('community was removed while moderation was pending');latest[roomIndex].banned=made.banned;save(latest);render();p.toast('member banned'); }catch(e){p.toast('member was not banned: '+(e&&e.message||e));} };
  ...
  async function webxdcCordParts(ctx){const p=PC(),room=saved().find(r=>roomIdentity(r)===ctx.room),reader=window.PosterCordReader,bundle=room&&room.cord&&room.cord.bundle,channel=room&&(room.channels||[]).find(c=>c.id===ctx.channelId||c.name===ctx.channel);if(!p||!room||!reader||!bundle||!channel)throw new Error('Concord Webxdc channel is unavailable');const loadKey=room.communityId||room.naddr,relays=[...new Set([...(bundle.relays||[]),...CORD_RELAYS])].slice(0,8);let controls=roomControls.get(loadKey);if(!controls){const seed=reader.inspectControl(bundle,[]);controls=await queryEnvelopeHistory(p,relays,seed.controlPubkeys,await cachedEnvelopes(envelopeCacheKey(loadKey,'control')));roomControls.set(loadKey,controls||[]);}const view=reader.inspectControl(bundle,controls||[]),wireChannel=view.channels.find(c=>c.id===channel.id);return{p,room,reader,bundle,channel,relays,controls:controls||[],streamPubkeys:wireChannel&&wireChannel.streamPubkeys||[]};}
  async function webxdcQuery(ctx,uuid){const p=PC();if(ctx.protocol==='nip29')return nip29RelayQuery(p,ctx.relay,[{kinds:[9450],'#h':[ctx.groupId],'#i':[uuid],limit:500}],10000);const x=await webxdcCordParts(ctx),wraps=await cordQuery(x.p,x.relays,[{kinds:[1059],authors:x.streamPubkeys,limit:1000}],{timeout:10000,max:8});return x.reader.inspectWebxdc(x.bundle,x.controls,x.channel.id,wraps,uuid,false);}
  async function webxdcPublish(ctx,uuid,content,meta,realtime){const p=PC(),tags=[['i',uuid],['alt',realtime?'Webxdc realtime':'Webxdc update']];if(realtime)tags.push(['rt','1']);for(const n of ['info','document','summary'])if(meta&&meta[n])tags.push([n,String(meta[n]).slice(0,200)]);if(ctx.protocol==='nip29')return p.publishNip29Authed(ctx.relay,{kind:realtime?24450:9450,created_at:Math.floor(Date.now()/1000),content,tags:[['h',ctx.groupId],...tags]});const x=await webxdcCordParts(ctx),viewer=x.p.viewer(),made=await x.reader.createWebxdcWrap(x.bundle,x.controls,x.channel.id,content,viewer.pubkey,x.p.signTemplate,tags,realtime);await x.p.relayPublishTo(x.relays,made.wrap);return made;}
  async function webxdcSubscribe(ctx,uuid,realtime,onEvent){const R=window.Relay;if(!R||!R.subscribeFrom)throw new Error('relay subscription unavailable');if(ctx.protocol==='nip29')return R.subscribeFrom([ctx.relay],[{kinds:[realtime?24450:9450],'#h':[ctx.groupId],'#i':[uuid],since:Math.floor(Date.now()/1000)-120}],{onEvent});const x=await webxdcCordParts(ctx),kind=realtime?21059:1059,filter={kinds:[kind],authors:x.streamPubkeys};if(realtime)filter.since=Math.floor(Date.now()/1000)-120;return R.subscribeFrom(x.relays,[filter],{onEvent:async wrap=>{try{const rows=await x.reader.inspectWebxdc(x.bundle,x.controls,x.channel.id,[wrap],uuid,realtime);for(const row of rows)onEvent(row);}catch(_){}}});}
  window.PCConcord={render,backgroundRender,openInvite:openInviteLink,openNotification,notificationRoute,inviteParts,normalizeIcon,roomIcon,reactionSummary,notifyMentions,discoverInvites,decodeMembershipLists,mergeArmadaBundle,nip29MembershipTags,nip29Memberships,nip29Metadata,nip29History,foldNip29History,nip29PreviousTags,publishNip29Message,syncNip29Memberships,hydrateNip29Room,threadParticipants,roomParticipants,typedMentionRecipients,textMentionsViewer,conversationIsVisible,repaintScrollTop,pendingEchoMatch,applyRoomIconMetadata,channelSectionsHtml,removeCommunityByIdentity,memberTapAction,memberViewportIsNarrow,encryptedAttachments,publicAttachments,messageContentHtml,wireRoomMedia,handoffState,acceptHandoff,beginComposerSend,restoreFailedComposer,webxdcOf,webxdcQuery,webxdcPublish,webxdcSubscribe};
