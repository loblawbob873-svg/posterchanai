...
(function(){
  ...
  if(!document.querySelector('link[data-concord-css]')){
    ...
    l.href='/static/css/concord.css?v=6'; (document.head||document.documentElement).appendChild(l);
  }
  ...
  function bind(me){
    ...
    const createGo=$('#cc-create-go'); if(createGo)createGo.onclick=()=>{ const name=String($('#cc-community-name').value||'').trim(); if(!name){ p.toast('name your community'); return; } const icon=normalizeIcon($('#cc-community-icon').value); const a=saved(); a.push({name,icon,channels:[{name:'general',private:false}],local:true,naddr:'local-'+Date.now().toString(36)}); save(a); state.community=a.length-1; state.channel='general'; render(); p.toast('local test community created'); };
    ...
    const send=$('#cc-send'); if(send&&input){
      send.onclick=()=>{ const text=String(input.value||'').trim(); const a=saved(), room=a[state.community]; if(!text)return; if(!room||!room.local){ p.toast('relay messaging becomes available after the invite is decrypted'); return; } const m=testMessages(room.naddr), viewer=p.viewer?p.viewer():{}; m.push({by:me,pubkey:viewer.pubkey||'',text,at:Date.now()}); saveTestMessages(room.naddr,m); render(); };
