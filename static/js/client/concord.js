...
(function(){
  ...
  if(!document.querySelector('link[data-concord-css]')){
    ...
    l.href='/static/css/concord.css?v=5'; (document.head||document.documentElement).appendChild(l);
  }
  ...
  function render(){
    ...
    feed.innerHTML=`<div class="cc-app">
      ...
      <main class="cc-conversation"><header><button class="cc-mobile-back" id="cc-back-channels" aria-label="Channels">‹</button><span class="cc-hash">#</span><b>${state.channel||'general'}</b><span class="cc-topic">${p.enc((current&&current.description)||'Public to community members')}</span><span class="cc-spacer"></span>${current?'<button class="cc-head-btn" id="cc-call" title="Start voice call"><svg class="ic"><use href="#i-phone"></use></svg></button>':''}<button class="cc-head-btn" id="cc-members" title="Members"><svg class="ic"><use href="#i-users"></use></svg></button></header>
  ...
  function bind(me){
    ...
    const settingsSave=$('#cc-settings-save'); if(settingsSave)settingsSave.onclick=()=>{ const a=saved(),room=a[state.community]; if(!room)return; room.description=String($('#cc-description-value').value||'').trim().slice(0,1000); room.icon=normalizeIcon($('#cc-settings-icon').value); save(a); render(); p.toast('community settings updated'); };
  ...
  window.PCConcord={render,inviteParts,normalizeIcon};
