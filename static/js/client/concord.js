...
(function(){
  ...
  function render(){
    ...
    feed.innerHTML=`<div class="cc-app">
      ...
      <main class="cc-conversation"><header><button class="cc-mobile-back" id="cc-back-channels" aria-label="Channels">‹</button><span class="cc-hash">#</span><b>${state.channel||'general'}</b><span class="cc-topic">Private Concord channel</span><span class="cc-spacer"></span>${current?'<button class="cc-head-btn" id="cc-call" title="Start voice call"><svg class="ic"><use href="#i-phone"></use></svg></button>':''}<button class="cc-head-btn" id="cc-members" title="Members"><svg class="ic"><use href="#i-users"></use></svg></button></header>
  ...
  function bind(me){
    ...
    const createGo=$('#cc-create-go'); if(createGo)createGo.onclick=()=>{ const name=String($('#cc-community-name').value||'').trim(); if(!name){ p.toast('name your community'); return; } const icon=normalizeIcon($('#cc-community-icon').value); const a=saved(); a.push({name,icon,local:true,naddr:'local-'+Date.now().toString(36)}); save(a); state.community=a.length-1; state.channel='general'; render(); p.toast('local test community created'); };
    const editIcon=$('#cc-edit-icon'); if(editIcon)editIcon.onclick=()=>{ const room=saved()[state.community]; $('#cc-icon-value').value=(room&&room.icon)||''; $('#cc-icon-dialog').classList.remove('hidden'); setTimeout(()=>$('#cc-icon-value').focus(),20); };
    ...
    const members=$('#cc-members'); if(members)members.onclick=()=>p.toast('Members: '+me);
