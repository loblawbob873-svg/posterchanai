...
(function(){
  ...
  function webxdcOf(m,room,channelName){
    ...
    for(const t of (m&&m.tags||[]).slice(0,100)){ if(t[0]!=='imeta')continue; const f={}; for(let i=1;i<Math.min(t.length,30);i++){ const s=String(t[i]||'').slice(0,4096),j=s.indexOf(' '); if(j>0)f[s.slice(0,j)]=s.slice(j+1); } if((f.m||'').toLowerCase()==='application/x-webxdc'&&/^https?:\/\//i.test(f.url||''))return {url:f.url,sha:f.x||'',uuid:f.webxdc||(scope?scope+'|webxdc':messageId(m)),name:(f.summary||f.name||'Mini app').slice(0,80),transport}; }
    /* Armada's missing-imeta-id fallback is the deterministic scope default, not the chat event id. */
    const url=(String(m&&m.text||'').slice(0,200000).match(/https?:\/\/[^\s<>]+\.xdc(?:\?[^\s<>]*)?/i)||[])[0]; return url?{url,sha:'',uuid:scope?scope+'|webxdc':messageId(m),name:'Mini app',transport}:null;
  }
