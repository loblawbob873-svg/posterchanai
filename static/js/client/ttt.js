...
(function(){
  function init(){
    ...
    async function render(){
      ...
      _timer = setInterval(()=>{ if(PC.VIEW==='ttt'){ _load(); } else clearInterval(_timer); }, 30000);
      ...
      const _bpk = safePk(PC.CFG.ttt_bot_npub);   // botNpub is the npub; the pool filters on hex
      if(_bpk && PC.watchBot) _botWatch = PC.watchBot(_bpk, ()=>{ if(PC.VIEW==='ttt') _load(); else { _botWatch && _botWatch(); _botWatch=null; } });
    }
