...
(function(){
  ...
  function openVmManager(){
    ...
    const paint=async()=>{if(dead||busy)return;busy=true;try{const r=await pcVM.list();if(dead)return;
      ...
      list.innerHTML=(r.machines||[]).map(m=>{const running=/running|paused|idle/.test(m.state);return `<article class="vmui-card"><div class="vmui-machine"><div class="vmui-screen ${running?'on':''}"><svg class="ic"><use href="#i-monitor"></use></svg></div><div><b>${enc(m.name)}</b><span><i class="${running?'on':''}"></i>${running?'Running':'Powered off'}</span><small>${enc(m.cpus)} processor${m.cpus===1?'':'s'} · ${enc(Math.round((m.memoryKiB||0)/1024))} MB memory</small></div></div><div class="vmui-actions">
