...
(function(){
  ...
  async function archivePart(p){
     ...
     * lost a race with memory pressure — was NEVER done: it was offered again on every migration
     * batch and republished on every pass, the migration could never be marked complete, and the
     * high-water mark was dragged back thirty days on every sweep behind it. Nothing was logged,
     * because from each pass's own point of view the upload succeeded.
     *
    ...
    try{
      for(const r of rows){
        ...
        if(!ok){
           ...
           * FIRST, so the wall is in front of everything newer than the bad row; and the sweep
           ...
           * — texts included — with a full Texts screen in front of the person, and the reason the
           * media never reached Blossom was that nothing after it was ever offered.
           *
  ...
  function migrateLocalHistory(){
    ...
    _fullMigration = (async () => {
      ...
      for(let batch=0; batch<1000; batch++){
         ...
         * turns the safety limit into the ordinary path rather than the impossible one. */
