...
(function(){
  ...
  function compute(posts, events, now, days){
    ...
    const byId=new Map(posts.map(p=>[p.id,{post:p,replies:0,reactions:0,reposts:0,zaps:0,score:0}]));
    events.forEach(e=>{ const row=byId.get(target(e)); if(!row)return;
      if(e.kind===1)row.replies++; else if(e.kind===7)row.reactions++; else if(e.kind===6)row.reposts++;
      else if(e.kind===9735){ const amount=(e.tags||[]).find(t=>t[0]==='amount'); row.zaps+=Math.round((Number(amount&&amount[1])||0)/1000); }
    });
    byId.forEach(r=>{ r.score=r.reactions+r.replies*2+r.reposts*3+(r.zaps?1:0); });
    ...
    const rows=[...byId.values()], totals=rows.reduce((a,r)=>({posts:a.posts+1,replies:a.replies+r.replies,reactions:a.reactions+r.reactions,reposts:a.reposts+r.reposts,zaps:a.zaps+r.zaps}),{posts:0,replies:0,reactions:0,reposts:0,zaps:0});
    totals.engagement=totals.replies+totals.reactions+totals.reposts; totals.rate=totals.posts?totals.engagement/totals.posts:0;
