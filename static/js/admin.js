...
async function loadSettings() {
    try {
        ...
        // Load WebDAV sync client config
        await loadWebDAVSyncConfig();
    } catch (err) {
        console.error('Failed to load settings:', err);
    }
}

// Load WebDAV sync client configuration
async function loadWebDAVSyncConfig() {
    try {
        const response = await fetch('/api/admin/webdav-sync-config');
        if (response.ok) {
            const config = await response.json();
            
            // Update cache enabled checkbox
            const cacheEnabled = document.getElementById('webdav_sync_cache_enabled');
            if (cacheEnabled) {
                cacheEnabled.checked = config.enable_cache !== false;
                toggleWebDAVCacheSettings(cacheEnabled.checked);
            }
            
            // Update cache settings
            if (config.cache_max_size_mb) {
                const sizeInput = document.getElementById('webdav_cache_max_size_mb');
                if (sizeInput) sizeInput.value = config.cache_max_size_mb;
            }
            
            if (config.cache_max_age_days) {
                const ageInput = document.getElementById('webdav_cache_max_age_days');
                if (ageInput) ageInput.value = config.cache_max_age_days;
            }
            
            if (config.cache_directories) {
                const dirsInput = document.getElementById('webdav_cache_directories');
                if (dirsInput) dirsInput.value = Array.isArray(config.cache_directories) 
                    ? config.cache_directories.join(', ') 
                    : config.cache_directories;
            }
            
            if (config.network_retry_attempts) {
                const retryInput = document.getElementById('webdav_network_retry_attempts');
                if (retryInput) retryInput.value = config.network_retry_attempts;
            }
            
            if (config.network_retry_delay) {
                const delayInput = document.getElementById('webdav_network_retry_delay');
                if (delayInput) delayInput.value = config.network_retry_delay;
            }
            
            const offlineMode = document.getElementById('webdav_offline_mode');
            if (offlineMode) {
                offlineMode.checked = config.offline_mode === true;
            }
        }
    } catch (err) {
        console.error('Failed to load WebDAV sync config:', err);
    }
}

// Toggle WebDAV cache settings visibility
function toggleWebDAVCacheSettings(enabled) {
    const settings = document.getElementById('webdav_cache_settings');
    const ageSettings = document.getElementById('webdav_cache_age_settings');
    const dirsSettings = document.getElementById('webdav_cache_dirs_settings');
    const networkSettings = document.getElementById('webdav_network_settings');
    const delaySettings = document.getElementById('webdav_network_delay_settings');
    const offlineSettings = document.getElementById('webdav_offline_settings');
    
    if (settings) settings.style.display = enabled ? 'block' : 'none';
    if (ageSettings) ageSettings.style.display = enabled ? 'block' : 'none';
    if (dirsSettings) dirsSettings.style.display = enabled ? 'block' : 'none';
    if (networkSettings) networkSettings.style.display = enabled ? 'block' : 'none';
    if (delaySettings) delaySettings.style.display = enabled ? 'block' : 'none';
    if (offlineSettings) offlineSettings.style.display = enabled ? 'block' : 'none';
}

// Refresh WebDAV directory list
async function refreshWebDAVDirectories() {
    try {
        const dirListDiv = document.getElementById('webdav_dir_list');
        const checkboxesDiv = document.getElementById('webdav_dir_checkboxes');
        if (!dirListDiv || !checkboxesDiv) return;
        
        dirListDiv.style.display = 'block';
        checkboxesDiv.innerHTML = '<div style="color: #888;">Loading directories...</div>';
        
        const response = await fetch('/api/files/list?path=');
        if (response.ok) {
            const data = await response.json();
            const items = data.items || [];
            const directories = items.filter(item => item.is_directory).map(item => item.name);
            
            checkboxesDiv.innerHTML = '';
            if (directories.length === 0) {
                checkboxesDiv.innerHTML = '<div style="color: #888;">No directories found</div>';
            } else {
                const currentDirs = document.getElementById('webdav_cache_directories').value
                ...
                directories.forEach(dir => {
                    ...
                    checkbox.value = dir;
                    checkbox.checked = currentDirs.includes(dir);
                    checkbox.style.marginRight = '8px';
                    
                    checkbox.addEventListener('change', () => {
                        updateCacheDirectoriesInput();
                    });
            ...
            checkboxesDiv.innerHTML = '<div style="color: #f44;">Failed to load directories</div>';
        }
    } catch (err) {
        console.error('Failed to refresh directories:', err);
        const checkboxesDiv = document.getElementById('webdav_dir_checkboxes');
        if (checkboxesDiv) {
            checkboxesDiv.innerHTML = '<div style="color: #f44;">Error: ' + err.message + '</div>';
        }
    }
}

// Update cache directories input from checkboxes
function updateCacheDirectoriesInput() {
    const checkboxes = document.querySelectorAll('#webdav_dir_checkboxes input[type="checkbox"]:checked');
    const dirs = Array.from(checkboxes).map(cb => cb.value);
    const dirsInput = document.getElementById('webdav_cache_directories');
    if (dirsInput) {
        dirsInput.value = dirs.join(', ');
    }
}

// Save WebDAV sync config
async function saveWebDAVSyncConfig() {
    try {
        const config = {
            enable_cache: document.getElementById('webdav_sync_cache_enabled').checked,
            cache_max_size_mb: parseInt(document.getElementById('webdav_cache_max_size_mb').value) || 10240,
            cache_max_age_days: parseInt(document.getElementById('webdav_cache_max_age_days').value) || 30,
            cache_directories: document.getElementById('webdav_cache_directories').value
                .split(',')
                .map(d => d.trim())
                .filter(d => d),
            network_retry_attempts: parseInt(document.getElementById('webdav_network_retry_attempts').value) || 5,
            network_retry_delay: parseInt(document.getElementById('webdav_network_retry_delay').value) || 5,
            offline_mode: document.getElementById('webdav_offline_mode').checked
        };
        ...
        if (response.ok) {
            ...
            alert('WebDAV sync client configuration saved! Restart the sync client daemon to apply changes.');
        } else {
            const error = await response.json();
            alert('Failed to save config: ' + (error.detail || 'Unknown error'));
        }
    } catch (err) {
        console.error('Failed to save WebDAV sync config:', err);
        alert('Error saving config: ' + err.message);
    }
}

// Setup WebDAV sync client event listeners
document.addEventListener('DOMContentLoaded', () => {
    const cacheEnabled = document.getElementById('webdav_sync_cache_enabled');
    if (cacheEnabled) {
        cacheEnabled.addEventListener('change', (e) => {
            toggleWebDAVCacheSettings(e.target.checked);
        });
        // Initial state
        toggleWebDAVCacheSettings(cacheEnabled.checked);
    }
    
    const refreshBtn = document.getElementById('refreshWebDAVDirsBtn');
    if (refreshBtn) {
        refreshBtn.addEventListener('click', refreshWebDAVDirectories);
    }
    
    // Add save button for WebDAV sync config
    const servicesTab = document.getElementById('tab-services');
    if (servicesTab) {
        const webdavFieldset = servicesTab.querySelector('fieldset:has(#webdav_sync_cache_enabled)');
        if (webdavFieldset) {
            const saveBtn = document.createElement('button');
            saveBtn.type = 'button';
            saveBtn.className = 'btn-primary';
            saveBtn.textContent = 'Save WebDAV Sync Config';
            saveBtn.style.marginTop = '10px';
            saveBtn.addEventListener('click', saveWebDAVSyncConfig);
            webdavFieldset.appendChild(saveBtn);
        }
    }
});
