...
from .keyboards import _build_torrent_keyboard, _has_nostr, _news_menu_keyboard, _split_news_into_articles, _strip_cmd_links, _strip_hashtags, _torrents_menu_keyboard, re
from .senders import User, _media_action_cache, _news_source_cache, _offer_social_post, _offer_ytdl_share, _offer_ytdl_video_actions, _send_active_torrents, _send_news_source_selector, _send_torrent_results, _strip_cmd_links, logger, re, telegram_service, time
...
async def _cb_nk(update, db, chat_id, data, callback_query, callback_query_id):
        ...
        if action == "post" and len(parts) >= 3:
            ...
            if not nk_user or not _has_nostr(nk_user):
                await telegram_service.send_message(chat_id, "⚠️ No Nostr key configured on your account.")
                return {"ok": True}
