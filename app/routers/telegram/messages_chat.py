...
from .keyboards import _has_matrix, _has_misskey, _has_nostr, _has_pleroma
from .senders import User, _has_matrix, _has_misskey, _has_pleroma, asyncio, datetime, logger, telegram_service
...
async def _msg_chat(attachments, chat_id, chat_service, command_service, db, doc_text, has_images, is_forwarded, message, reply_text, text, user_obj):
                ...
                if youtube_url and (is_forwarded or not text_stripped.replace(youtube_url, '').strip()):
                    ...
                    if _has_misskey(_yt_user_for_social) or _has_pleroma(_yt_user_for_social) or _has_matrix(_yt_user_for_social) or _has_nostr(_yt_user_for_social):
                        yt_keyboard.append([
                ...
                if _x_dl and (is_forwarded or not text_stripped.replace(_x_orig, '').strip()):
                    ...
                    if _has_misskey(_x_user_for_social) or _has_pleroma(_x_user_for_social) or _has_matrix(_x_user_for_social) or _has_nostr(_x_user_for_social):
                        x_keyboard.append([
                            {"text": "📣 Post", "callback_data": "yt:post"}
                        ])
