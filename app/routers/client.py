...
# ----- one-time migration: hand the OLD Budget Manager's rows to the client -----
# The Budget doc is encrypted to the user's own Nostr key, so only their browser can write it — there
# is no server-side import, by design. This endpoint therefore does the one half the server CAN do:
# read the legacy SQLite file and hand the rows over, so a phone can migrate without moving a file
# around. The client encrypts and publishes them (budget.js importDialog).
#
# It is OFF unless data/budget_import.json exists (data/ is gitignored), and that file names the
# token, the finance account and the DB path:
#     {"token": "...", "user": "someone@example.com", "db": "/home/you/finance/budget.db"}
# Gating on a token in a local file rather than on the caller's identity is deliberate: the legacy DB
# holds SEVERAL people's finances and there is no trustworthy npub → finance-username mapping, so
# "whoever holds the token gets exactly the one named account" is the only rule that can't leak
# someone else's bills. DELETE THE FILE when the migration is done and the endpoint disappears.
class BudgetLegacyReq(BaseModel):
    token: str


@router.post("/budget/legacy")
async def budget_legacy(data: BudgetLegacyReq):
    import json as _json
    import sqlite3 as _sq

    cfg_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data", "budget_import.json")
    try:
        with open(cfg_path) as fh:
            cfg = _json.load(fh)
    except Exception:
        raise HTTPException(status_code=404, detail="legacy import is not enabled")

    want = str(cfg.get("token") or "")
    # compare_digest, not ==: a plain comparison leaks the token prefix through timing.
    if not want or not hmac.compare_digest(want, data.token or ""):
        raise HTTPException(status_code=403, detail="bad token")

    db_path = cfg.get("db") or ""
    user = cfg.get("user") or ""
    if not os.path.exists(db_path):
        raise HTTPException(status_code=404, detail="legacy database not found on this node")

    # immutable=1 — never take a write lock or create a -wal beside a DB the old app may still own.
    con = _sq.connect(f"file:{db_path}?immutable=1", uri=True)
    con.row_factory = _sq.Row
    row = con.execute("SELECT id FROM users WHERE username=?", (user,)).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="that account is not in the legacy database")
    uid = row["id"]

    bills = [{"name": b["name"], "cost": b["cost"], "paid": b["paid"],
              "payment_method": b["payment_method"] or "",
              "is_income": bool(b["is_income"]), "is_recurring": bool(b["is_recurring"]),
              "hidden_month": b["hidden_month"] or ""}
             for b in con.execute(
                 "SELECT * FROM bills WHERE user_id=? ORDER BY is_income DESC, sort_order", (uid,))]
    cats = [{"id": c["id"], "name": c["name"], "paid": c["paid"],
             "hidden_month": c["hidden_month"] or ""}
            for c in con.execute(
                "SELECT * FROM plan_categories WHERE user_id=? ORDER BY sort_order", (uid,))]
    items = [{"cat": i["category_id"], "name": i["name"], "amount": i["amount"]}
             for i in con.execute(
                 "SELECT * FROM plan_items WHERE user_id=? ORDER BY sort_order", (uid,))]
    con.close()
    logger.info("[budget] legacy export served for %s (%d bills)", user, len(bills))
    return JSONResponse({"bills": bills, "cats": cats, "items": items})
