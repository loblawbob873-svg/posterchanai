...
_HELP_SECTIONS = {
    ...
    "files": (
        "📎 *Files — compress, convert, meme, dildo, poo, cum & blood*\n\n"
        ...
        "• You can also skip the buttons: send the file with `compress`, `clip 0:10 0:30`, `convert`, `meme <text>`, `dildo`, `poo`, `cum` or `blood` as the caption.\n"
...
def _media_effects_keyboard() -> dict:
    ...
    transforms (meme caption, dildo / poo / cum / blood scatter), plus a Back to
    the main actions."""
...
async def _handle_telegram_update(update: dict, db: Session):
    ...
    try:
        ...
        if message:
            ...
            command = None
            arg = text
            commands = ["help", "new", "ytdl", "geni", "mail", "news", "search", "images", "yt", "torrents", "nyaa", "4chan", "logs", "translate", "post", "share", "compress", "clip", "convert", "meme", "dildo", "poo", "cum", "blood", "node", "budget", "finance", "bills", "pay", "addbill", "screenshot", "shot", "ss"]
            for cmd in commands:
            ...
            if has_images and attachments and command not in ("compress", "clip", "convert", "meme", "dildo", "poo", "cum", "blood"):
                for filename, file_data, content_type in attachments:
                    if content_type.startswith("image/"):
                        import base64
                        image_b64 = base64.b64encode(file_data).decode('utf-8')
            ...
            if oversized_attachment and command in ("compress", "clip", "convert", "meme", "dildo", "poo", "cum", "blood", None):
