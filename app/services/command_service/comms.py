...
class _CommsMixin:
    ...
    async def _post_command(self, arg: str, attachments: Optional[list]) -> dict:
        """Share text (and an optional attached image) to Nostr — the web equivalent of the Telegram
        📣 Post flow. The built-in relay federates it, and this node's fediverse server sends it on to
        fediverse followers; a clear message when no Nostr key is linked."""
        ...
        if not has_nostr:
            return {"type": "text", "content": (
                "No Nostr key linked. Link one in **Settings → Nostr**, then `post <text>`.")}
        ...
        if not text and not img_bytes:
            return {"type": "text", "content": (
                "Usage: `post <text>` — optionally attach an image to share it too. Goes to Nostr "
                "(and on to the fediverse).")}
