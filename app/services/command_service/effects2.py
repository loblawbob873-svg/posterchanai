...
class _Effects2Mixin:
    ...
    async def _hugebitch_command(self, attachments: Optional[list]) -> dict:
        """Turn an attached image into a short MP4 set to the Huge Bitch clip: `hugebitch`."""
        from app.services.media_service import is_image

        if not attachments or not any(is_image(fn, ct) for fn, _, ct in attachments):
            return {"type": "text", "content": "Attach an image, then send `hugebitch`."}

        import asyncio
        from app.services.effects_service import hugebitch_attachments

        outputs, summary = await asyncio.to_thread(hugebitch_attachments, attachments)
        if not outputs:
            return {"type": "text", "content": summary}
        return {"type": "files", "content": summary, "files": outputs}
