...
THE RULES, each enforced here on the host:
  ...
  * Sessions persist in `.state/sessions.json` (0600) so an app restart does not strand every client.
"""
...
class SessionRegistry:
    def __init__(self, path: Path | None = None, now=time.time):
        ...
        self.now = now
        self._s: dict = {}                     # session pk -> {"owner", "exp", "scope", "closed"}
    ...
    def _prune(self) -> None:
        t = self.now()
        for k in [k for k, v in self._s.items() if v["exp"] + GRACE < t]:
            self._s.pop(k, None)
    ...
    def _persist(self) -> None:
        ...
        try:
            ...
            with os.fdopen(fd, "w", encoding="utf-8") as f:
                json.dump({"v": 1, "sessions": self._s}, f)
    ...
    def lookup(self, session_pk: str):
        ...
        with self._lock:
            rec = self._s.get(str(session_pk or "").lower())
            if rec is None:
                return None, None
            if rec["closed"] or rec["exp"] <= self.now():
    ...
    def open(self, owner: str, session_pk: str, exp: int) -> None:
        with self._lock:
            ...
            live = [k for k, v in self._s.items() if v["owner"] == owner and not v["closed"]
                    and v["exp"] > self.now() and k != session_pk]
            if len(live) >= MAX_PER_OWNER:
                oldest = min(live, key=lambda k: self._s[k]["exp"])
                self._s[oldest]["closed"] = True
            self._s[session_pk] = {"owner": owner, "exp": int(exp), "scope": "use", "closed": False}
            self._persist()
    ...
    def close(self, owner: str, session_pk: str | None = None) -> int:
        with self._lock:
            ...
            for k, v in self._s.items():
                if v["owner"] == owner and not v["closed"] and (session_pk is None or k == session_pk):
                    v["closed"] = True
                    n += 1
            if n:
                self._persist()
...
class SessionOps:
    ...
    async def _op_session_close(self, pk, role, args, progress):
        ...
        return {"closed": self._sessions().close(pk, target)}
