...
def _broadcastable(ev, cfg=None) -> bool:
    ...
    if k == 30078:
        ...
        if d.startswith("pcai:"):
            # NOTE: mail (`pcai:mail:`) is deliberately NOT broadcast here. Fanning the mailbox out to
            # the instance's PUBLIC upstream relays floods them with encrypted blobs they reject (the
            # outbox queue backs up → heavy CPU). The mailbox lives on the home relay and the web
            # client reads it there directly. Real cross-relay portability must target the USER's own
            # relays (NIP-65), not the public upstream set — a separate, controlled path.
