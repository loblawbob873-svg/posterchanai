...
def _read_config() -> dict:
    ...
    try:
        ...
        cfg = {
            ...
            # GRASP git-over-nostr (NIP-34): 30618 repo state (push token), 1617 patch, 1621 issue,
            # 1622 reply, 1623 repo-reply, 1630-1633 issue/patch status. These ride the firehose so a
            # repo THIS node hosts sees external collaboration; acceptance is repo-SCOPED (see
            # _firehose_event) so it's not an open spam firehose. Purge-safe (not in _PRUNABLE_KINDS).
