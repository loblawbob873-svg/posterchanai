...
# ---- peer-encrypted docs (bridge → user): the FEDERATED model -------------------------------------
# Unlike the docs above (self-encrypted to the signer, local-only), these are NIP-44-encrypted TO a
# *recipient's* key with `encrypt_to` and `p`-tagged to them, so the RECIPIENT decrypts them with
# their own key from ANY relay — the basis of the portable, federated mailbox. The signer (a server-
# held "bridge" key) can also read them back: NIP-44's conversation key is ECDH, hence symmetric.

def _decode_from(content: str, reader_sk: bytes, counterparty_pubkey: str):
    """Decrypt a peer-encrypted doc (reader is either side of the ECDH) + JSON-parse it."""
    try:
        raw = nip44.decrypt_from(reader_sk, bytes.fromhex(counterparty_pubkey), content)
    except Exception:
        return None
    try:
        return json.loads(raw)
...
async def put_doc_to(port: int, signer_sk: bytes, peer_pubkey: str, d_tag: str, data,
                     *, tags: list | None = None, kind: int = APP_KIND) -> bool:
    """Create/replace `d_tag` signed by `signer_sk`, NIP-44-encrypted TO `peer_pubkey` and p-tagged to
    them. Federates (the relay's _broadcastable lets the mail namespace through to the paced outbox)."""
    payload = data if isinstance(data, str) else json.dumps(data, separators=(",", ":"))
    content = nip44.encrypt_to(signer_sk, bytes.fromhex(peer_pubkey), payload)
    ev = build_event(signer_sk, kind, content, tags=[["d", d_tag], ["p", peer_pubkey]] + (tags or []))
    ok, msg = await _ws_publish(port, ev)
    if not ok:
        logger.warning("[nostr-store] put_to %s rejected: %s", d_tag, msg)
    return ok


async def get_doc_from(port: int, author_pubkey: str, d_tag: str, *, reader_sk: bytes,
                       counterparty_pubkey: str, kind: int = APP_KIND):
    """Read a peer-encrypted doc `d_tag` authored by `author_pubkey`, decrypting as `reader_sk`."""
    evs = await _ws_query(port, [{"authors": [author_pubkey], "kinds": [kind], "#d": [d_tag], "limit": 1}])
    if not evs:
        return None
    evs.sort(key=lambda e: e.get("created_at", 0), reverse=True)
    return _decode_from(evs[0].get("content", ""), reader_sk, counterparty_pubkey)


async def list_docs_from(port: int, author_pubkey: str, prefix: str, *, reader_sk: bytes,
                         counterparty_pubkey: str, p_tag: str | None = None, kind: int = APP_KIND) -> dict:
    """{d_tag: data} for peer-encrypted docs under `prefix` authored by `author_pubkey` (optionally
    narrowed to those p-tagged to `p_tag`), newest per d_tag, decrypted as `reader_sk`."""
    flt = {"authors": [author_pubkey], "kinds": [kind], "limit": 5000}
    if p_tag:
        flt["#p"] = [p_tag]
    ...
    for ev in evs:
        ...
        if d not in best or ev.get("created_at", 0) > best[d].get("created_at", 0):
            best[d] = ev
    return {d: _decode_from(ev.get("content", ""), reader_sk, counterparty_pubkey) for d, ev in best.items()}


async def list_dtags_from(port: int, author_pubkey: str, prefix: str, *, p_tag: str | None = None,
                          kind: int = APP_KIND) -> set:
    """d-tags under `prefix` authored by `author_pubkey` (optionally p-tagged to `p_tag`) — NO decrypt.
    For cheap dedup where the key is in the d-tag itself (mailbox UID checks)."""
    flt = {"authors": [author_pubkey], "kinds": [kind], "limit": 5000}
    if p_tag:
        flt["#p"] = [p_tag]
    evs = await _ws_query(port, [flt])
