...
| Installed 1.0.979 account gate | Authenticated installed Electron rendered 29 folders and 30 entries; server and client indexes matched at 5,983 files. Both registered sync roots matched server = manifest = local (11,954 and 5,820) with zero skipped. Office/WOPI/editor returned HTTP 200 | Passed against installed package over loopback-only CDP |
| Post-deploy full gate | `./test.sh --live https://poster.place`: 4,024 unit tests, 1,615 client tests, and 62/64 browser/live checks initially passed. The two red checks reproduced clean alone: profile/search stability passed 20/20 cold flows and QR device login passed clock skew, idle socket, two-app and reload/signing scenarios. The cause was release-gate contention: the 20-cold-browser stability test was in the six-way live batch. `2d0d063e` serializes both relay-heavy gates and `test_checkall_live_isolation.py` locks that policy. The installed-account skip was explicitly rerun and passed as recorded above. CSS scale remains an advisory (498 values) | Functional failures closed; repeat the complete isolated gate after further code changes |
