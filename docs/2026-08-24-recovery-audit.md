...
| Office service | Installed Gentoo 1.0.979 created an authenticated temporary `.txt` session, verified WOPI read/write/read, attached CDP to Collabora's out-of-process iframe, and required a complete document workspace with real controls before deleting the session. The same run retained exact Files/sync counts; `check_installed_desktop_account.py` makes the path repeatable | Installed authenticated service and rendered interactive editor verified; typing through Collabora and observing its autosave remains open |
