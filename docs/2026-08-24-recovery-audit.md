...
Installer commits `5d8a61aeb`, `9412d2943`, `73a969607`, `732d95ce8` and `e7f18120d` add a
fresh fail-fast finalizer and correct the installed-chroot target contract; the public overlay now
exposes the resulting shell as 1.0.20260828101723 without changing the Desktop client artifact.

| Requirement | Current authoritative evidence | State / missing proof |
|---|---|---|
| Source regression gates | At the final package/ISO head, the complete client suite passed **1,834 tests plus 99 subtests** in 471.00 seconds. The complete non-client suite passed **4,313 tests plus 473 subtests** in 331.00 seconds, with 15 explicit environment skips and no failure. After the installer-only commits, `checkall.py` again passed the 344-second unit suite, the 816-second client suite and every runnable browser/package check. Its sole red result was the installed shell-surface probe invoking `swaymsg` on this headless host; that prerequisite is now an explicit named skip, with six focused tests and runner classification passing | Green for runnable source/package behavior. Eleven installed/hardware argument gates remain explicit skips and must not be read as passes; installed, authenticated and physical-device gates remain separate |
...
| Desktop artifact / Gentoo | Immutable Desktop 1.0.1141 Linux is 152,222,202 bytes, SHA-256 `f69903ef09ee274a5421875aca56457059912a46f294174faf7aa6c96602859d`; its Manifest records SHA-512 `ace81d3757605d1b9d4ae170feaae2160cc06dcd79d5a324b2d0ac7aa54bc55903e550bbf702c6fccb0fa16c8efd894740afb49ff5a4d0737d58b5fe206fdbfb`. The public overlay exposes Desktop 1.0.1141 and shell 1.0.20260828101723. That shell owns `/usr/local/libexec/posterchanos/publish_iso.sh`, and installed `gentoo.sh` selects it instead of the invalid historical `/usr/scripts` path. The final installer/profile families pass 72 and 53 tests respectively | Published. The reachable desktop installed both versions and the installed publisher contract passed. The laptop was unreachable and remains unverified |
...
## 2026-08-28 post-`5f5a3426e` evidence matrix

This audits commits after `5f5a3426e` through local head `f9e4f3e02`. A source/client test is not an
installed or hardware result. The successful `c4e917cb2` artifacts do not contain later commits
`35b802f7a`, `ac6586faf`, or `f9e4f3e02`.

| Backlog requirement | New authoritative evidence | Proven boundary | Still required |
|---|---|---|---|
| Texts history and MMS state | `987d38243` keeps the newest live archive version; `f5078d5d9` queues video MMS from desktop; `294376ad8` prevents cancelled outbox requests resurfacing. Android build/emulator workflows succeeded at `c4e917cb2` | Source/runtime and API-34 emulator | Representative long-history/media migration, real carrier sends/callbacks and retry/cancel/delete without cross-surface ghosts on a physical phone |
| Messages/Concord recovery | `f6182eae4` adds cold history restore and bottom-scroll stability; `f167e9844` fixes desktop mention polling; `3be633367` paints cached history immediately; `34347f281` clears the composer after send. Desktop workflow `33212295126` passed its bundled-Concord audit | Source/client plus bundle audit at `c4e917cb2` | Installed monitor handoff and live Armada two-identity history/send/reply/react/mention/moderation, notifications, icons and attachment/Webxdc interop |
| Files, Code, Preview and Office | `25fc200f9` safely opens user-selected local folders; `34c9ef4ef` preserves picker extensions; `59ff73123` refreshes Files after upload. Installed disposable Files/Preview and Code/Terminal gates are recorded above. `ac6586faf` hardens isolation of the installed Office check | Focused source/package and the specifically recorded installed disposable paths | Authenticated final-head Files/index/sync completeness, folder-upload visibility, arbitrary user project flow and Collabora HTTP-200 edit/save/readback on the final installed package |
| Android app launch and Folder Sync | `1678134ec` prevents an old 10k+ entry cache paint from doing a full network/decrypt pass; `f25e976a3` opens every shipped view in a real WebView; `f9e4f3e02` holds that large-cache test through delayed paint/startup work. Workflow `33212295089` completed lifecycle and instrumented checks at `c4e917cb2` | Exact-head API-34 emulator at `c4e917cb2`; later delayed-work test compiled locally | Artifact containing `f9e4f3e02` on a physical phone with representative old/large sync data, including background/resume and external document providers |
| Android boot/launcher/media | `a7e84f38a` prevents late boot from replacing an app-owned view; exact `c4e917cb2` lifecycle/instrumented workflow succeeded | API-34 emulator | Physical/OEM launcher, double-Home, audible Music-after-Home, Bluetooth, portrait/landscape/tablet and notification deep links |
| Terminal, Email and Posterfetch | `bfda5de2f` restores pinned terminal viewport after idle handoff; `d647c3c50` hardens responsive Email attachment reading; `3a9a1deab` reports every GPU model | Focused source/browser/package gates | Installed focus/resize/monitor movement, real attachment URLs on desktop/mobile and representative multi-GPU hardware |
| Window/app ownership and Settings | `25447bb00` makes installed Code focus account-independent; `c4e917cb2` exercises all installed Settings pages across focus. Desktop workflow `33212295126` built and published Linux/Windows/macOS from exact `c4e917cb2` | Artifact build/publish plus specifically recorded disposable installed interactions | Exhaustive every-app focus/resize/handoff matrix, native Firefox/Telegram, black-window, clipboard, snap and sustained Foot streaming remain blockers |
| Android signing/store | `b64006f26` removed the committed keystore; `fb25a193a`, `c4c95de93` and `ed260d11e` guard secrets/rotation. Workflow `33212295043` passed secret rejection, signed build, GitHub and Zapstore publication at `c4e917cb2` | CI signing/publication | User-visible Zapstore install/update across supported old/rotated lineages; later commits require a newly identified APK |
| Tor/proxy reboot | Live boot logs proved Postgres recovery was swallowed while Tor stayed active without 9052/9062. `35b802f7a` rejects unhydrated role startup, restores canonical boot defaults and makes proxy pull in Tor; 52 focused tests passed | Reproduced live failure plus source/unit fix | Deploy/reboot and verify 9052/9062/8118/8119 plus outbound traffic without manual restart; fix is newer than cited artifacts |
| LiveCD/installer | `92eb125f1` adds a Wayland GPU to the smoke VM; `62f2ba5c3` supports x86-64-v3 under TCG; `6f7f4a3d7` makes switch-root/session setup deterministic | Structural/source checks and serial switch-root progress | Current graphical gate still reports inactive display output: rebuild, prove stable graphical boot, install/eject/reboot, then physical USB boot before publication |
| Release boundary | Desktop `33212295126`, emulator `33212295089` and Android `33212295043` all succeeded at exact `c4e917cb2` | Identified successful CI boundary | Local head is newer and other active lanes are dirty/unshipped. Final candidate needs clean exact-head suite, artifacts, Gentoo pin, installed laptop/dual-monitor/phone checks, then ISO |
...
| macOS desktop option | Runtime tests execute tray reparenting in both styles. The installed `.102` capture shows only application icons in the bottom-centre Dock, with machine status, relay, notification and clock controls separately in the top-right menu. The top-left bar contains PosterChan, Task Manager and Full Screen without a duplicate standalone System Settings item | Installed layout verified. The dropdown's two actions and exhaustive multi-monitor/theme rollback interaction remain open |
