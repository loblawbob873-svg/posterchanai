...
snapshots() {
	...
	if [[ "$CURRENT_ROOT" == *"snapshot"* ]]; then
		...
		echo "Removing Snapshots older than 5 days"
...
partitionDetection() {
	...
	echo "BTRFS Mapper Name: $ROOT_MAPPER_NAME"
	echo "BTRFS Encrypted Volume: $BTRFS"
	echo "BTRFS Subvolume: $ROOT_NAME"
	echo "--------------------------------------------"
	echo
}
...
updateOS() {
    ...
    /usr/bin/emerge -c
    if lspci -vvv | grep -i nvidia | grep -i VGA > /dev/null; then
        echo "NVIDIA found!"
...
backupOS() {
	clear
	echo
	echo "[Backup OS to Build Server via Rsync]"
	echo
	clear
...
tweaks() {
	...
	echo
	echo "[1] Chroot into existing OS"
	echo "[2] Enable/Disable Disk Password at Boot"
...
menu() {
	...
	if [[ $choice = 1 ]]; then
		...
		echo
		echo "[Repository Choice]"
		echo
		echo "Do you want to use your local repo or the official Gentoo Repo?"
		echo
