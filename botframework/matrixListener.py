...
def process_messages():
    ...
    for message in messages:
        ...
        if _attachment and _attachment.get("mxc_url"):
            ...
            if not prompt_text.strip():
                ...
                if _is_img:
                    ...
                    _opts.append("• `redeem` — turn the image into a do not redeem video")
                    _opts.append("• `gigity` — turn the image into a giggity video")
                    _opts.append("• `beavis` — turn the image into a Beavis laugh video")
                    _opts.append("• `smell` — turn the image into a can you imagine the smell video")
                    _opts.append("• `hood` — turn the image into a 10s hood video")
                    _opts.append("• `akbar` — turn the image into an akbar video")
                    _opts.append("• `retard` — turn the image into a retard-alert video")
                    _opts.append("• `whoabuddy` — turn the image into a whoa buddy video")
                    _opts.append("• `sopranos` — turn the image into a Sopranos theme video")
                    _opts.append("• `cheers` — turn the image into a Cheers theme video")
                    _opts.append("• `munsters` — turn the image into a Munsters theme video")
                    _opts.append("• `happydays` — turn the image into a Happy Days theme video")
                    _opts.append("• `dontwanttowait` — turn the image into a Dawson's Creek theme video")
                    _opts.append("• `strangerthings` — turn the image into a Stranger Things theme video")
                    _opts.append("• `adamsfamily` — turn the image into an Addams Family theme video")
                    _opts.append("• `xmen` — turn the image into an X-Men theme video")
                    _opts.append("• `futurama` — turn the image into a Futurama theme video")
                    _opts.append("• `charliesangles` — turn the image into a Charlie's Angels theme video")
                    _opts.append("• `differentstroke` — turn the image into a Diff'rent Strokes theme video")
                    _opts.append("• `seinfeld` — turn the image into a Seinfeld theme video")
                    _opts.append("• `onepiece` — turn the image into a One Piece theme video")
                    _opts.append("• `overtaken` — turn the image into an overtaken video")
                    _opts.append("• `freebird` — turn the image into a Free Bird video")
                    _opts.append("• `kanye` — turn the image into a Kanye video")
                    ...
                    _opts.append("• `harlem` — turn the image into a Harlem Shake video")
                    _opts.append("• `chimp` — overlay the animated chimp gif on the lower third")
                    _opts.append("• `consider` — overlay the 'consider the following' cutout on an image")
                    _opts.append("• `clay` — overlay the background-removed Clay Davis 'Shiiiit' clip")
                    _opts.append("• `wasteland` — turn the image into a Teenage Wasteland video")
                    _opts.append("• `mixalot` — turn the image into a Baby Got Back video")
        ...
        # which processes them and posts the results back into this room.
                ...
                or lower_prompt == "redeem" or lower_prompt.startswith("redeem ") \
                or lower_prompt == "gigity" or lower_prompt.startswith("gigity ") \
                or lower_prompt == "beavis" or lower_prompt.startswith("beavis ") \
                or lower_prompt == "smell" or lower_prompt.startswith("smell ") \
                or lower_prompt == "hood" or lower_prompt.startswith("hood ") \
                or lower_prompt == "akbar" or lower_prompt.startswith("akbar ") \
                or lower_prompt == "retard" or lower_prompt.startswith("retard ") \
                or lower_prompt == "whoabuddy" or lower_prompt.startswith("whoabuddy ") \
                or lower_prompt == "sopranos" or lower_prompt.startswith("sopranos ") \
                or lower_prompt == "cheers" or lower_prompt.startswith("cheers ") \
                or lower_prompt == "munsters" or lower_prompt.startswith("munsters ") \
                or lower_prompt == "happydays" or lower_prompt.startswith("happydays ") \
                or lower_prompt == "dontwanttowait" or lower_prompt.startswith("dontwanttowait ") \
                or lower_prompt == "strangerthings" or lower_prompt.startswith("strangerthings ") \
                or lower_prompt == "adamsfamily" or lower_prompt.startswith("adamsfamily ") \
                or lower_prompt == "xmen" or lower_prompt.startswith("xmen ") \
                or lower_prompt == "futurama" or lower_prompt.startswith("futurama ") \
                or lower_prompt == "charliesangles" or lower_prompt.startswith("charliesangles ") \
                or lower_prompt == "differentstroke" or lower_prompt.startswith("differentstroke ") \
                or lower_prompt == "seinfeld" or lower_prompt.startswith("seinfeld ") \
                or lower_prompt == "onepiece" or lower_prompt.startswith("onepiece ") \
                or lower_prompt == "overtaken" or lower_prompt.startswith("overtaken ") \
                or lower_prompt == "freebird" or lower_prompt.startswith("freebird ") \
                or lower_prompt == "kanye" or lower_prompt.startswith("kanye ") \
                ...
                or lower_prompt == "harlem" or lower_prompt.startswith("harlem ") \
                or lower_prompt == "chimp" or lower_prompt.startswith("chimp ") \
                or lower_prompt == "consider" or lower_prompt.startswith("consider ") \
