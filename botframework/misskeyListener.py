...
_BOT_HELP_TEXT = (
    ...
    "• news <source> — headlines (e.g. news drudge)\n"
    "• geni <prompt> — generate an image\n"
    "• ytdl <url> — download audio; ytdl video <url> for video (YouTube/X). "
    "Add clip <start> <end> and/or compress, e.g. ytdl video <url> clip 0:10 0:30 compress\n"
    ...
    "• cum — scatter cum all over an attached image\n"
    "• blood — splatter blood all over an attached image\n"
    "• bullethole — punch bullet holes into an attached image\n"
    "• fire — set an attached image on fire\n"
    "• gay — stamp a big red GAY on an attached image\n"
    "• blacked — slap the BLACKED logo on an attached image\n"
    "• kosher — stamp a 100% KOSHER seal on an attached image\n"
    "• barked — drop a smirking dog + #BARKED on an attached image\n"
    "• hava — turn an attached image into a 6s Hava Nagila video\n"
    "• indian — turn an attached image into a 6s Indian-song video\n"
    ...
    "• fahh — turn an attached image into a fahh video\n"
    "• helpme — turn an attached image into a 5s helpme video\n"
    "• gong — turn an attached image into a gong video\n"
    "• fbi — turn an attached image into an FBI open up video\n"
    "• redeem — turn an attached image into a do not redeem video\n"
    "• gigity — turn an attached image into a giggity video\n"
    "• beavis — turn an attached image into a Beavis laugh video\n"
    "• smell — turn an attached image into a can you imagine the smell video\n"
    "• hood — turn an attached image into a 10s hood video\n"
    "• akbar — turn an attached image into an akbar video\n"
    "• retard — turn an attached image into a retard-alert video\n"
    "• whoabuddy — turn an attached image into a whoa buddy video\n"
    "• sopranos — turn an attached image into a Sopranos theme video\n"
    "• cheers — turn an attached image into a Cheers theme video\n"
    "• munsters — turn an attached image into a Munsters theme video\n"
    "• happydays — turn an attached image into a Happy Days theme video\n"
    "• dontwanttowait — turn an attached image into a Dawson's Creek theme video\n"
    "• strangerthings — turn an attached image into a Stranger Things theme video\n"
    "• adamsfamily — turn an attached image into an Addams Family theme video\n"
    "• xmen — turn an attached image into an X-Men theme video\n"
    "• futurama — turn an attached image into a Futurama theme video\n"
    ...
    "• harlem — turn an attached image into a Harlem Shake video\n"
    "• chimp — overlay the animated chimp gif on the lower third of an image\n"
    "• consider — overlay the 'consider the following' cutout on an image\n"
    "• clay — overlay the background-removed Clay Davis 'Shiiiit' clip on an image\n"
    "• wasteland — turn an attached image into a Teenage Wasteland video\n"
    "• mixalot — turn an attached image into a Baby Got Back video\n"
...
def _handle_media_command(note, command, arg, own_acct, visibility):
    ...
    if not media:
        send_reply(note, "📎 Attach a file to your post, then add `compress`, `clip 0:10 0:30`, `convert`, `meme <text>` or `dildo`.",
