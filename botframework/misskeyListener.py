...
def _handle_media_command(note, command, arg, own_acct, visibility):
    """Run compress/clip/convert on a note's attachment(s) via the backend and
    post the result file(s) back. Shared shape with the Pleroma listener."""
    media = _gather_note_media(note)
    if not media:
        send_reply(note, "📎 Attach a file to your post, then add `compress`, `clip 0:10 0:30` or `convert`.",
